...
### E-47 - Origin Gate adaptif untuk origin HTTPS, Cloudflare Tunnel, dan jalur publik
...
- **Hasil sesudah perbaikan:** release Docker pada `20260911T112013Z` lulus. Healthcheck publik, enam container, public storage, bundle frontend, dan WebSocket upgrade lulus pada Integration Gate. Security Gate memeriksa jalur publik dan menghasilkan **0 gagal, 0 peringatan**, termasuk HSTS aktif pada HTTPS. Backup konfigurasi `.env` dan profile Docker sebelum perubahan tersimpan dengan permission `600` di `/home/ServerDeploy/backups/sita-cloudflare-tunnel/20260911T111627Z`.

### E-49 - Isolasi diagnosis Tunnel dan kesiapan origin aaPanel
...
### E-52 - Otomatisasi perbaikan capability PHP aaPanel

- **Temuan:** PHP 8.4 aaPanel di VM lab dibangun dengan flag `--disable-fileinfo`; extension tidak tersedia sebagai `fileinfo.so`, padahal SITA memakai validasi MIME untuk unggahan dokumen dan avatar.
- **Uji kelayakan terisolasi:** source PHP aaPanel pada `/www/server/php/84/src.tar.gz` diekstrak ke direktori sementara. `phpize`, `./configure --enable-fileinfo=shared`, dan `make -j1` menghasilkan `fileinfo.so` berukuran 8.7 MB. PHP 8.4 berhasil memuat modul tersebut pada pengujian `php -n -d extension=<modul> -m`, tanpa mengubah runtime aktif.
- **Rancangan perbaikan:** tahap Bootstrap sekarang mendeteksi fileinfo yang kurang, menghitung pemakai `enable-php-84.conf` pada vhost aaPanel, dan hanya melakukan build/pemasangan otomatis bila runtime dipakai satu vhost. Script mencadangkan `php.ini` dan modul lama, memasang modul ke extension directory PHP aktif, reload melalui init script aaPanel, dan memverifikasi modul. Pada runtime multi-vhost ia berhenti kecuali persetujuan eksplisit diberikan.
- **Validasi end-to-end:** pada 13 September 2026, fileinfo dipulihkan lebih dahulu ke kondisi tidak aktif dari backup, kemudian script dijalankan sebagai akun deploy dengan sudo. Script menemukan satu vhost PHP 8.4, menyimpan backup di `/var/backups/sita/php-extensions/20260913T155221Z-php84`, membangun modul, reload PHP-FPM melalui init script, dan selesai dengan exit `0`. Pemeriksaan CLI serta konfigurasi PHP-FPM masing-masing menemukan `fileinfo`; master PHP-FPM tetap aktif.

### E-53 - Root aaPanel bawaan sebagai control checkout atomic

- **Temuan:** aaPanel membuat situs lab dengan root standar `/www/wwwroot/sita-aapanel.ikydev.com;` dan file operasional seperti `.user.ini`, `.htaccess`, serta halaman error. Memaksa source masuk ke subfolder `sita` membuat operator perlu mengubah root vhost manual sebelum deploy awal.
- **Perbaikan:** atomic runner sekarang menerima tiga keadaan root yang terverifikasi: root bawaan aaPanel `APP_DIR`, root control checkout `APP_DIR/public`, atau root release `CURRENT_LINK/public`. Pada dua keadaan pertama, runner membuat backup vhost, menguji sintaks Nginx, lalu mengalihkan hanya directive root ke release aktif. Profile baru memakai strategi atomic sebagai default.
- **Makna skripsi:** otomatisasi yang baik menyesuaikan kondisi awal platform yang nyata, bukan meminta operator membentuk server agar cocok dengan asumsi skrip. Guard tetap menolak root yang tidak dapat diidentifikasi agar vhost lain tidak berubah secara tidak sengaja.

### E-54 - Wizard instalasi awal aaPanel yang menjaga pemisahan GUI, source, dan secret

- **Masalah operasional:** prosedur awal memerlukan sejumlah perintah manual untuk izin folder, Git, pembuatan `.env`, kunci aplikasi, kunci Reverb, dan profile. Banyak langkah meningkatkan peluang salah path, lupa permission, atau secret tertulis pada riwayat terminal.
- **Perbaikan:** `deploy/aapanel-first-install.sh` menyediakan alur terminal interaktif satu kali. Operator hanya memasang komponen melalui GUI aaPanel, membuat Website dan database/user, lalu menjalankan wizard. Wizard menerima domain/path/branch dan kredensial database secara tersembunyi, mempertahankan file bawaan aaPanel, checkout source, membuat `.env` mode `640`, menghasilkan `APP_KEY` dan credential Reverb acak, serta membuat profile mode `600` tanpa secret.
- **Pemisahan tanggung jawab:** aaPanel tetap menjadi sumber konfigurasi Website, PHP, database, dan monitoring. Wizard dan Deployment Console mengelola source SITA, konfigurasi aplikasi, service, gate, release, backup, serta rollback. Profile menyimpan identitas operasional server yang dapat digunakan ulang; `.env` tetap menjadi satu-satunya lokasi nilai rahasia.
- **Validasi awal:** skrip lulus `bash -n`. Uji instalasi penuh pada VM baseline dilakukan setelah operator memasukkan kredensial database langsung di terminal; secret tidak dicatat pada log eksperimen atau repository.

### E-55 - Penyederhanaan alur: clone source sebagai satu-satunya entry point terminal

- **Evaluasi rancangan:** wizard yang diunduh terpisah memang dapat mengurangi jumlah perintah, tetapi menambah artefak dan versi eksternal yang harus dipercaya operator. Hal ini tidak sejalan dengan kebutuhan operasional kampus: operator seharusnya cukup memakai source repository yang telah disetujui.
...
### E-58 - Bootstrap atomic pada database aaPanel baru

- **Kasus awal:** pada VM aaPanel hasil reset, Bootstrap pertama berhenti berurutan karena Node belum tersedia, koneksi registry npm terputus, lalu Laravel melaporkan `Migration table not found`. Ketiga kondisi terjadi sebelum candidate dapat menjadi release aktif.
- **Perbaikan:** Check dapat memasang Node 22 terisolasi setelah persetujuan; pemasangan dependency candidate diulang maksimal tiga kali pada folder candidate; dan database tanpa tabel `migrations` diperlakukan sebagai Bootstrap awal. Pada kondisi terakhir operator menyetujui migration, runner membuat dump MySQL/MariaDB terkompresi serta checksum terlebih dahulu, menjalankan migration dari candidate, lalu memeriksa ulang status migration. Migration tertunda pada update biasa tetap memblokir aktivasi candidate.
- **Bukti keamanan:** backup awal tersimpan dengan mode privat di `/var/backups/sita/` sebagai berkas `.sql.gz` dan `.sha256`; file kredensial client sementara dibersihkan oleh trap pada semua jalur gagal. Tidak ada password database dicatat pada log atau repository.

### E-59 - Integrasi vhost aaPanel dan validasi origin Tunnel

- **Kasus awal:** vhost baru dari GUI aaPanel memakai root standar dan include PHP, tetapi belum memiliki `try_files` Laravel maupun proxy Reverb. Setelah root dialihkan ke candidate, endpoint `/up` masih `404`, sehingga Atomic Release melakukan rollback. Pada percobaan sebelumnya, healthcheck publik juga menghasilkan `530` karena `cloudflared` tidak ada setelah reset VM.
- **Perbaikan:** runner memasang fragment milik SITA pada `extension/<domain>/sita-integration.conf`, jalur include bawaan aaPanel. Fragment berisi rewrite Laravel, public storage, proxy Reverb lokal, deny path sensitif, dan header keamanan. Vhost utama—domain, SSL, log, serta pilihan PHP—tetap dikelola GUI aaPanel. Pemasangan fragment selalu menjalankan `nginx -t`, membuat backup jika fragment sebelumnya ada, dan memulihkan fragment bila sintaks gagal.
- **Hasil uji akhir, 13 September 2026:** Bootstrap `aapanel-bootstrap-20260913T182701Z.log` mengaktifkan release `20260913T182704Z-76a4f4c550a4`. Integration Gate memilih origin HTTP loopback dengan header `Host`, memperoleh healthcheck `200`, membuktikan public storage, dan mendapatkan WebSocket upgrade `101`. Reverb, queue, dan scheduler aktif; synchronization check lulus; Security Gate menghasilkan `0 gagal, 1 peringatan`.
- **Batas hasil:** jalur publik `https://sita-aapanel.ikydev.com/up` masih `530` karena connector Cloudflare Tunnel belum dipasang kembali setelah reset. Ini tercatat sebagai peringatan `EDGE_ACCESS_POLICY=warn`; pada server yang mewajibkan akses publik sebelum rilis, nilai profile dapat diubah menjadi `required`. HSTS juga belum dapat diuji end-to-end ketika gate memilih origin HTTP; verifikasi tersebut dilakukan setelah Tunnel atau origin HTTPS aktif.

### E-60 - Uji browser dua akun membuktikan chat realtime publik tanpa refresh

- **Kondisi uji:** 14 September 2026, deployment aaPanel aktif di `https://sita-aapanel.ikydev.com`. Dua browser context Playwright dipisahkan: `mahasiswa@sita.test` dan `dosen@sita.test`. Keduanya membuka thread Bimbingan yang sama sebelum pesan dikirim.
...
### E-63 - Pemeriksaan hak administrasi sebelum menjalankan gate

- **Temuan pada VPS aaPanel bersih:** proses instalasi panel dapat mengubah atau memuat ulang komponen sistem yang dipakai `sudo`. Pada satu pemeriksaan selama instalasi, akun SSH deploy sementara tidak dapat menjalankan `sudo` noninteraktif. Jika hal ini terjadi setelah runner mulai, kegagalan akan muncul terlambat saat perubahan vhost, permission, atau service dilakukan.
- **Perbaikan:** `deploy/aapanel-release.sh` sekarang memeriksa akses administrasi sebelum fase pertama. Operator `root` langsung diterima. Operator non-root diverifikasi dengan `sudo -v` pada terminal interaktif atau `sudo -n true` pada pemanggilan noninteraktif. Bila tidak memenuhi, runner berhenti dengan instruksi untuk menjalankan sebagai root atau menyiapkan hak sudo.
- **Batas keamanan:** pemeriksaan tidak menambah hak akses akun dan tidak mengubah `sudoers`. Ini hanya mencegah deployment berjalan setengah jalan dengan privilege yang tidak memadai.

### E-64 - Replikasi deploy awal pada VPS aaPanel baru

- **Kondisi uji:** VPS Ubuntu 22.04 baru dengan 2 vCPU, 2 GB RAM, dan 20 GB disk. aaPanel 8.0.6 dipasang, lalu operator membuat Website PHP `sita-vps.ikydev.com` dan database/user `sita_vps` melalui GUI. DNS publik sengaja belum diarahkan ke VPS; gate memakai origin HTTP lokal dan mencatat jalur publik sebagai peringatan.
- **Temuan prosedur:** folder Website baru dimiliki `www`, sehingga `git clone` oleh akun SSH biasa ditolak. Clone penuh juga dapat terputus pada koneksi terminal pendek. Prosedur diperbaiki menjadi clone `sudo` dengan `--depth=1`, lalu kepemilikan diberikan kepada operator dan grup `www`. Checkout branch berhasil pada commit `1c26259`.
- **Temuan runtime:** Check awal mendeteksi `fileinfo` tidak aktif, Node/npm belum tersedia, serta permission `storage` dan `bootstrap/cache` belum siap. Perbaikan fileinfo dijalankan otomatis hanya setelah memastikan PHP-FPM dipakai satu vhost; backup PHP dibuat di `/var/backups/sita/php-extensions/`. Node 22 dipasang di `/opt/sita/node` dengan checksum resmi diverifikasi. Sebelumnya Bootstrap memeriksa permission runtime sebelum tahap yang menyiapkannya; runner diperbaiki agar menyiapkan direktori runtime sebelum sinkronisasi.
- **Hasil deploy:** Bootstrap membuat candidate atomic, membuat backup database terverifikasi sebelum migration awal, mengaktifkan `current/public` dengan backup vhost, serta mengaktifkan Reverb, queue, dan scheduler. Integration Gate lulus untuk origin HTTP, storage, PHP-FPM, service, frontend Reverb, dan WebSocket upgrade. Security Gate mencatat 0 gagal serta 1 peringatan HSTS karena edge HTTPS/DNS belum disiapkan.
- **Batas interpretasi:** bukti ini menunjukkan kesiapan origin aaPanel dan konsistensi alur Bootstrap pada VPS baru. Uji domain publik, edge HTTPS, dan chat dua akun dilakukan setelah DNS atau Tunnel lab diarahkan ke VPS; hasilnya dicatat terpisah agar tidak mencampur origin dengan edge.

### E-65 - Replikasi pada aaPanel yang telah memiliki situs SITA lain

- **Kondisi uji:** 14 September 2026, pada VPS aaPanel yang telah menjalankan `sita-vps.ikydev.com`, operator membuat Website dan database kedua melalui GUI aaPanel: `sita-retest.ikydev.com` dan `sita_retest`. Instalasi kedua menggunakan source checkout, Inisialisasi aplikasi, dan Bootstrap yang sama tanpa menghapus situs pertama maupun memasang ulang PHP/Node global.
- **Temuan penting:** kedua aplikasi awalnya dapat memakai port Reverb internal bawaan `127.0.0.1:8080`. Reverb aplikasi kedua gagal bind karena port sudah dipakai aplikasi pertama. Pada rancangan lama, probe WebSocket generik dapat keliru mencapai aplikasi pertama dan memberi hasil lulus semu. Ini adalah potensi *false negative* nyata pada topologi multi-situs.
- **Perbaikan:** profile baru memilih port Reverb internal deterministik yang bebas pada rentang `18000-18999`; profile lama yang berkonflik dimigrasikan sebelum release. Port tersebut diselaraskan pada profile, control checkout, shared environment atomic, unit systemd, dan fragment proxy Nginx. Service Reverb, queue, serta scheduler sekarang menjadi syarat wajib aktivasi release; kegagalan service menyebabkan release gagal sehingga mekanisme atomic dapat mengembalikan source dan konfigurasi yang dicadangkan.
- **Hasil validasi:** release kedua pada commit `e1f03aa` mengaktifkan candidate `20260914T131002Z-e1f03aaf7f7c`. Reverb situs pertama tetap mendengar pada `127.0.0.1:8080`, sedangkan Reverb situs kedua aktif pada `127.0.0.1:18776`. Nginx situs kedua terbukti mem-proxy ke port `18776`; healthcheck origin HTTP, public storage, seluruh service, bundle frontend, dan WebSocket upgrade lulus. Synchronization check juga mengonfirmasi `.env` control checkout dan shared environment memiliki port yang sama.
- **Peringatan yang benar:** PHP-FPM 8.4 digunakan dua vhost sehingga gate hanya memberi peringatan dan tidak memasang/menghapus extension PHP otomatis. Hal ini mencegah perubahan global pada situs lain. Jalur DNS publik belum diarahkan pada domain retest sehingga probe publik menghasilkan peringatan; origin lokal tetap lulus dan tidak diklaim sebagai uji edge HTTPS.
- **Makna skripsi:** deployment yang terlihat berhasil pada satu aplikasi tidak cukup menjadi bukti keandalan ketika satu panel mengelola banyak situs. Gate harus menguji identitas backend yang benar, isolasi resource runtime, dan layanan aktif agar tidak menyatakan release siap hanya karena layanan situs lain merespons.

### E-66 - Baseline normal T1 sebelum eksperimen gangguan

- **Versi dan lingkungan:** Docker telah diselaraskan ke commit `eb7e4bc` (perubahan fungsional Console terakhir `e1f03aa`); aaPanel retest memakai release `20260914T131002Z-e1f03aaf7f7c`. Kedua environment memuat mekanisme gate yang sama.
- **Prosedur:** mode `check` Console dijalankan lima kali berurutan pada kondisi tanpa gangguan, tanpa perubahan source atau konfigurasi selama pengulangan.
...
### E-69 - Replikasi edge Cloudflare proxy langsung pada VPS aaPanel

- **Tujuan:** menyamakan lab VPS aaPanel dengan pola produksi kampus yang memakai origin HTTPS publik di belakang Cloudflare, bukan Cloudflare Tunnel.
- **Konfigurasi:** record `A` proxied `sita-retest.ikydev.com` diarahkan ke VPS `103.52.114.214`. Sertifikat Cloudflare Origin dibuat untuk hostname tersebut; private key dibuat dan disimpan hanya pada VPS dengan mode privat. Vhost aaPanel kemudian mendengar pada port `443`, memakai TLS 1.2/1.3 dan HSTS. Konfigurasi vhost lama dicadangkan di `/var/backups/sita/nginx/` sebelum perubahan.
- **Gate:** profile diubah dari mode origin HTTP/Tunnel menjadi `HTTP_PROBE_MODE=origin-https`, `CHECK_EDGE_HTTP=true`, dan `EDGE_ACCESS_POLICY=required`. Integration Gate sekarang mewajibkan dua bukti: healthcheck origin HTTPS dengan TLS/SNI yang benar dan healthcheck publik melalui Cloudflare.
- **Penguatan origin:** snapshot UFW dibuat di `/var/backups/sita/firewall/`; allow umum port 80/443 diganti dengan allowlist rentang IPv4/IPv6 resmi Cloudflare. SSH dan port aaPanel tidak diubah. Akses langsung dari jaringan penguji ke `103.52.114.214:443` kemudian timeout, sedangkan request melalui Cloudflare menghasilkan HTTP `200`, header `Server: cloudflare`, serta HSTS aktif.
- **WebSocket:** handshake publik menuju endpoint Reverb menghasilkan HTTP `101`. Curl kemudian timeout karena koneksi WebSocket memang ditahan terbuka; status `101` adalah bukti upgrade yang relevan.
- **Hasil:** Check aaPanel pada `20260914T154400Z` lulus untuk sinkronisasi, precheck, origin HTTPS, edge Cloudflare publik, PHP-FPM, Reverb, queue, scheduler, bundle frontend, dan WebSocket. Mode SSL zona Cloudflare tetap `Full` karena mengubahnya menjadi `Full (strict)` berdampak pada seluruh subdomain `ikydev.com`; tidak diklaim setara strict sebelum origin lain diaudit.

- **Validasi sesudah perbaikan:** pengulangan ulang G-01 aaPanel pada commit `a9be7d7` menghasilkan exit `1` dalam `12.466 ms` saat Reverb dihentikan. Integration Gate melaporkan service tidak aktif setelah sepuluh pemeriksaan dan WebSocket HTTP `404`. Sesudah service dinyalakan kembali, Check lulus dalam `8.336 ms`. Hasil ini menjadi pengulangan resmi pertama G-01 aaPanel.
