...
printf 'timestamp_utc,scenario,expected,gate_exit,detection,duration_ms\n' > "$REPORT_FILE"
unexpected=0
...
run_case() {
    ...
    printf '%s,%s,%s,%s,%s,%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$scenario" "$expected" "$exit_code" "$detected" "$duration" >> "$REPORT_FILE"
    printf '%-30s expected=%-4s detected=%-5s duration_ms=%s\n' "$scenario" "$expected" "$detected" "$duration"
...
if [[ -r "$NGINX_CONFIG" ]]; then
    ...
    printf '%s,nginx_permissions_policy_missing,fail,not-run,not-run,0\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$REPORT_FILE"
    printf '%-30s expected=fail detected=not-run duration_ms=0\n' 'nginx_permissions_policy_missing'
    unexpected=$((unexpected + 1))
fi
